    ...
    try
    {
        ...
        switch (ci.chromatogramType)
        {
            ...
            case MS_photodiode_array_detector:
            {
                ...
                pwiz::msdata::TimeIntensityPair* _data = reinterpret_cast<pwiz::msdata::TimeIntensityPair*>(cd->data());
                if (getBinaryData) result->setTimeIntensityPairs(_data, cd->size(), UO_minute, MS_number_of_detector_counts);
                else result->defaultArrayLength = cd->size();
            }
            break;
        }
